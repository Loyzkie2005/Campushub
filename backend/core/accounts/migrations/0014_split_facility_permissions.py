...
def split_facility_permissions(apps, schema_editor):
    ...
    book_perm = permission_map.get("can_access_facilities")
    manage_perm = permission_map.get("can_manage_facilities")
    if book_perm:
        book_perm.name = "Can book facilities"
        book_perm.save(update_fields=["name"])
    ...
    for role in Role.objects.all().prefetch_related("granted_permissions"):
        ...
        if role_lower in {"super admin", "superadmin"} and manage_perm:
            ...
            current.add("can_manage_facilities")
        elif "facility" in role_lower:
            if manage_perm:
                role.granted_permissions.remove(manage_perm)
            current.discard("can_manage_facilities")
            if book_perm:
                role.granted_permissions.add(book_perm)
                current.add("can_access_facilities")
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("accounts", "0013_role_granted_permissions"),
    ]
    ...
    operations = [
        ...
        migrations.RunPython(split_facility_permissions, migrations.RunPython.noop),
    ]
